...
export default function UnifiedAuthPage() {
  ...
  const getInputClassName = (hasError: boolean = false) => {
    const baseClass = "block w-full pl-10 pr-3 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:border-transparent";
    if (hasError) {
      return `${baseClass} border-red-300 focus:ring-red-500 bg-red-50`;
    }
    return `${baseClass} border-gray-300 focus:ring-yellow-500`;
  };

  // Helper function for password fields (with right padding for eye icon)
  const getPasswordInputClassName = (hasError: boolean = false) => {
    const baseClass = "block w-full pl-10 pr-12 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:border-transparent";
    if (hasError) {
      return `${baseClass} border-red-300 focus:ring-red-500 bg-red-50`;
    }
    return `${baseClass} border-gray-300 focus:ring-yellow-500`;
